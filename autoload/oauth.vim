...
function! oauth#access_token(url, ctx, ...)
  ...
  let a:ctx.edam_noteStoreUrl = http#decodeURI(substitute(filter(split(res.content, "&"), "v:val =~ '^edam_noteStoreUrl='")[0], '^[^=]*=', '', ''))
  let a:ctx.edam_userId = http#decodeURI(substitute(filter(split(res.content, "&"), "v:val =~ '^edam_userId='")[0], '^[^=]*=', '', ''))
  return a:ctx
endfunction
